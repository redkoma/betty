...
class Scrb051ReportLeave extends StatefulWidget {
  const Scrb051ReportLeave({Key? key}) : super(key: key);

  @override
  _Scrb051ReportLeaveState createState() => _Scrb051ReportLeaveState();
}

class _Scrb051ReportLeaveState extends State<Scrb051ReportLeave> {
  Map arguments = {};
  ...
  List<Widget> _buildRows() {
    ...
    return List.generate(
      listUser.length + 1,
      (index) => Row(
        children: (index == 0) ? _buildHeaderColumn(AppStyle().session['company']['leave_type']) : _buildColumn(listUser[index - 1], AppStyle().session['company']['leave_type']),
      ),
    );
  }
    ...
    await FirebaseFirestore.instance
      ...
      if (querySnapshot.size > 0) {
        for (var doc in querySnapshot.docs) {
          ...
          tmp['id'] = doc.id;
          obj['docinfo'][tmp['uid']] ??= {};
          obj['docinfo'][tmp['uid']][tmp['data']['leave_type']] ??= {'sum': 0.0, 'count': 0.0};
          obj['docinfo'][tmp['uid']][tmp['data']['leave_type']]['count'] += 1;
          obj['docinfo'][tmp['uid']][tmp['data']['leave_type']]['sum'] += tmp['data']['day'];
        }
      }
      setState(() {});
    });
