...
class Scrb057ReportExpenseByProject extends StatefulWidget {
  const Scrb057ReportExpenseByProject({Key? key}) : super(key: key);

  @override
  _Scrb057ReportExpenseByProjectState createState() => _Scrb057ReportExpenseByProjectState();
}

class _Scrb057ReportExpenseByProjectState extends State<Scrb057ReportExpenseByProject> {
  Map arguments = {};
  ...
  List<Widget> _buildRows() {
    ...
    return List.generate(
      ...
      (index) => Row(
        children:
            (index == 0) ? _buildHeaderColumn(AppStyle().session['company']['expense_type']) : _buildColumn(listUser[index - 1], AppStyle().session['company']['expense_type']),
      ),
    );
  }
    ...
    return List.generate(
      ...
      (index) => InkWell(
        ...
        child: Container(
          ...
          padding: EdgeInsets.all(4.0),
          child: Text("${(((obj['docinfo'] ?? {})[user['uid']] ?? {})[listCell[index]['name']] ?? {})['sum'] ?? ''}"),
        ),
      ),
    );
  }
    ...
    await FirebaseFirestore.instance
      ...
      if (querySnapshot.size > 0) {
        for (var doc in querySnapshot.docs) {
          ...
          tmp['id'] = doc.id;
          if ((tmp['project'] ?? '') == (obj['params']['project'] ?? '')) {
            obj['docinfo'][tmp['uid']] ??= {};
            obj['docinfo'][tmp['uid']][tmp['data']['expense_type']] ??= {'sum': 0.0, 'count': 0.0};
            obj['docinfo'][tmp['uid']][tmp['data']['expense_type']]['count'] += 1;
            obj['docinfo'][tmp['uid']][tmp['data']['expense_type']]['sum'] += tmp['data']['expense_amount'];
          }
        }
      }
      setState(() {});
