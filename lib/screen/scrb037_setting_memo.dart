...
class Scrb037MemoSetting extends StatefulWidget {
  const Scrb037MemoSetting({Key? key}) : super(key: key);

  @override
  _Scrb037MemoSettingState createState() => _Scrb037MemoSettingState();
}

class _Scrb037MemoSettingState extends State<Scrb037MemoSetting> {
  Map arguments = {};
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  onTap: () async {
                    var res0 = await Navigator.pushNamed(context, '/scrb027', arguments: {'view_as': obj['users'][index]});
                    _refresh();
                  },
                );
              },
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Builder(
                builder: (context) {
                  List<Widget> list = [];
                  list = (obj['memo_type'] ?? []).map<Widget>((map) {
              ...
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb040', arguments: {'memo_type': obj['memo_type']});
                if (res != null) {
                  obj['memo_type'] = res['data'];
                  await FirebaseFirestore.instance.collection('company').doc(AppStyle().session['data']['uid']).set({'memo_type': obj['memo_type']}, SetOptions(merge: true));
                  AppStyle().session['company']['memo_type'] = obj['memo_type'];
                  setState(() {});
                }
              },
            ),
  ...
  void initData() async {
    obj['workflow'] = {
      ...
      'code': 'l1',
      'cover': 'assets/images/memo_setting_001.png',
    };

    obj['users'] = [];

    _refresh();
    if (AppStyle().session['company']['memo_type'] == null) {
      AppStyle().session['company']['memo_type'] = AppStyle().session['setting']['default_memo_type'];
      await FirebaseFirestore.instance
          .collection('company')
          .doc(AppStyle().session['data']['uid'])
          .set({'memo_type': AppStyle().session['setting']['default_memo_type']}, SetOptions(merge: true));
    }
    obj['memo_type'] = AppStyle().session['company']['memo_type'];
