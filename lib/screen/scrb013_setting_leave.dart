...
class Scrb013LeaveSetting extends StatefulWidget {
  const Scrb013LeaveSetting({Key? key}) : super(key: key);

  @override
  _Scrb013LeaveSettingState createState() => _Scrb013LeaveSettingState();
}

class _Scrb013LeaveSettingState extends State<Scrb013LeaveSetting> {
  Map arguments = {};
  ...
  Widget build(BuildContext context) {
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  onTap: () async {
                    var res0 = await Navigator.pushNamed(context, '/scrb024', arguments: {'view_as': obj['users'][index]});
                    _refresh();
                  },
                );
              },
        ...
        child: Column(
          children: [
            ListTile(
              title: Text('Workflow'),
              trailing: Icon(Icons.keyboard_arrow_right),
              subtitle: Text('Single Approval [HR]'),
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb014', arguments: {'data': obj['workflow']});
                if (res != null) {
                  obj['workflow'] = res['data'];
                  var flow = {'leave': obj['workflow']};
                  ...
                  AppStyle().session['company']['workflow']['leave'] = obj['workflow'];
                  setState(() {});
                }
              },
            ),
            ...
            Divider(height: 1),
            ListTile(
              title: Text('Final Approve User'),
              trailing: Icon(Icons.keyboard_arrow_right),
              subtitle: Text('${AppStyle().session['company']['leave_final_username'] ?? 'Please select'}'),
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb017', arguments: {'data': obj['ori_users']});
                if (res != null) {
                  AppStyle().session['company']['leave_final_username'] = res['data']['userinfo']['displayName'] ?? res['data']['userinfo']['email'];
                  AppStyle().session['company']['leave_final_user'] = res['data'];
                  await FirebaseFirestore.instance.collection('company').doc(AppStyle().session['data']['uid']).set({
                    'leave_final_username': AppStyle().session['company']['leave_final_username'],
                    'leave_final_user': AppStyle().session['company']['leave_final_user'],
            ...
            ListTile(
              ...
              subtitle: Builder(
                builder: (context) {
                  List<Widget> list = [];
                  list = (obj['leave_type'] ?? []).map<Widget>((map) {
                    return (map['enable'] ?? false)
                        ? Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
              ...
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb016', arguments: {'leave_type': obj['leave_type']});
                if (res != null) {
                  obj['leave_type'] = res['data'];
                  await FirebaseFirestore.instance.collection('company').doc(AppStyle().session['data']['uid']).set({'leave_type': obj['leave_type']}, SetOptions(merge: true));
                  AppStyle().session['company']['leave_type'] = obj['leave_type'];
                  setState(() {});
                }
              },
            ),
    ...
    var tabSummary = Container(
      ...
      child: Column(
        children: [
          ListTile(
            title: Text("สรุปการลารายเดือน"),
            onTap: () {
              Navigator.pushNamed(context, '/scrb051', arguments: {'title': "สรุปการลารายเดือน", 'params': {}});
            },
          ),
          ListTile(
            title: Text("สรุปเข้างานรายเดือน"),
            onTap: () {
              Navigator.pushNamed(context, '/scrb052', arguments: {'title': "สรุปเข้างานรายเดือน", 'params': {}});
            },
          ),
        ],
      ),
  ...
  void initData() async {
    ...
    _refresh();
    if (AppStyle().session['company']['leave_type'] == null) {
      AppStyle().session['company']['leave_type'] = AppStyle().session['setting']['default_leave_type'];
      await FirebaseFirestore.instance
          .collection('company')
          .doc(AppStyle().session['data']['uid'])
          .set({'leave_type': AppStyle().session['setting']['default_leave_type']}, SetOptions(merge: true));
    }
    obj['leave_type'] = AppStyle().session['company']['leave_type'];
    setState(() {});
  }
