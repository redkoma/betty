...
class Scrb041TrainingSetting extends StatefulWidget {
  const Scrb041TrainingSetting({Key? key}) : super(key: key);

  @override
  _Scrb041TrainingSettingState createState() => _Scrb041TrainingSettingState();
}

class _Scrb041TrainingSettingState extends State<Scrb041TrainingSetting> {
  Map arguments = {};
  ...
  Widget build(BuildContext context) {
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  onTap: () async {
                    var res0 = await Navigator.pushNamed(context, '/scrb026', arguments: {'view_as': obj['users'][index]});
                    _refresh();
                  },
                );
              },
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Divider(height: 1),
            ListTile(
              title: Text('Confirm Training User'),
              trailing: Icon(Icons.keyboard_arrow_right),
              subtitle: Text('${AppStyle().session['company']['training_confirm_username'] ?? 'Please select'}'),
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb017', arguments: {'data': obj['ori_users']});
                if (res != null) {
                  AppStyle().session['company']['training_confirm_username'] = res['data']['userinfo']['displayName'] ?? res['data']['userinfo']['email'];
                  AppStyle().session['company']['training_confirm_user'] = res['data'];
                  await FirebaseFirestore.instance.collection('company').doc(AppStyle().session['data']['uid']).set({
                    'training_confirm_username': AppStyle().session['company']['training_confirm_username'],
                    'training_confirm_user': AppStyle().session['company']['training_confirm_user'],
            ...
            ListTile(
              ...
              subtitle: Builder(
                builder: (context) {
                  List<Widget> list = [];
                  list = (obj['training_type'] ?? []).map<Widget>((map) {
                    return (map['enable'] ?? false)
                        ? Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
              ...
              onTap: () async {
                dynamic res = await Navigator.pushNamed(context, '/scrb042', arguments: {'training_type': obj['training_type']});
                if (res != null) {
                  obj['training_type'] = res['data'];
                  await FirebaseFirestore.instance
                      .collection('company')
                      .doc(AppStyle().session['data']['uid'])
                      .set({'training_type': obj['training_type']}, SetOptions(merge: true));
                  AppStyle().session['company']['training_type'] = obj['training_type'];
                  setState(() {});
                }
              },
            ),
    ...
    var tabSummary = Container(
      ...
      child: Column(
        children: [
          ListTile(
            title: Text("สรุปการอบรมรายเดือน"),
            onTap: () {
              Navigator.pushNamed(context, '/scrb054', arguments: {'title': "สรุปการอบรมรายเดือน", 'params': {}});
            },
          ),
        ],
      ),
    ...
    return DefaultTabController(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text('Training Request Settings'),
  ...
  void initData() async {
    ...
    _refresh();
    if (AppStyle().session['company']['training_type'] == null) {
      AppStyle().session['company']['training_type'] = AppStyle().session['setting']['default_training_type'];
      await FirebaseFirestore.instance
          .collection('company')
          .doc(AppStyle().session['data']['uid'])
          .set({'training_type': AppStyle().session['setting']['default_training_type']}, SetOptions(merge: true));
    }
    obj['training_type'] = AppStyle().session['company']['training_type'];
